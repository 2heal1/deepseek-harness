...
`run(input, { sessionId?, onNotification? })` 拥有一个 submission：它取得 `{ messageId, submissionId }`，收集到匹配的持久 settlement，然后返回 `RunResult { sessionId, finalResponse, events, notifications }`。`finalResponse` 是该 submission 区间内根会话最后提交的 assistant 文本。`events` 包含根会话事件，`notifications` 还包含通过 `subagent.started` 发现的后代，均按协议传输顺序排列。传输丢失、超时和协议违例会导致 Promise 被拒绝；settlement reason 保留在事件流中。

## HarnessClient

自有运行 API 之下的协议客户端：显式 `start()`/`initialize()`/`prompt()`/`request()`/`close()`，外加通知订阅。`prompt()` 返回已接纳的 message 与 submission id，不等待执行。`subscribe(filter?)` 返回 `NotificationSubscription`（可等待的 `next()`、非阻塞 `tryNext()`、异步迭代）；`subscribeSessionTree(id)` 把范围限定到一个会话及从 `subagent.started` 血缘边发现的后代——运行时对上下文内每个会话都发通知，范围限定在客户端完成，与 Python SDK 完全一致。本包导出有明确类型的错误：`JsonRpcResponseError`（协议错误响应，保留 code/data）、`RequestTimeoutError`（配置的时限已到）、`SdkProtocolError`（响应超出文档化协议）、`TransportClosedError`（运行时已消失——消息携带退出码与有界 stderr 尾部）。
...
- **没有提示词取消**——receipt settlement 已精确关联，但协议没有定向取消方法；放弃 submission 意味着关闭运行时。
