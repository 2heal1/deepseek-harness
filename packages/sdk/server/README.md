...
`initialize` is the runtime-readiness boundary: when the server is mounted by a Loader composition, it waits for the current plugin tree to settle before replying, so async sibling capabilities such as initial MCP tool discovery are visible to the first prompt. Hand-built contexts without Loader remain immediately usable. `initialize.serverInfo.name` is the wire-stable `deepseek-harness-sdk-runtime`, and version `0.0.2` identifies the receipt protocol. An optional positive `initialize.maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants; invalid values reject initialization, while omission sends no SDK cap and allows the selected adapter or provider route default to apply. `session/prompt` submits one identified user message and immediately returns `{ messageId, submissionId }`. The server streams every durable fact as `session.event`; clients use the matching submission lifecycle records to delimit the accepted work. Independent requests may enqueue more work on the same session. Persistence roots and persona come from `cordis.yml`.
...
- **Settlement is event-streamed** — `session/prompt` acknowledges acceptance; clients wait for the matching durable submission settlement rather than receiving a second unary result.
