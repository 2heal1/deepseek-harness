...
网关还拥有 `runtimeStatus`，即 `agent/runtime/facts` 的 last-wins 投影。其值为最新通过校验的 `AgentRuntimeFacts` 或 `null`，通过同一 history baseline 与 `session/projection` push 路径提供；它报告 runtime identity 与 phase，而不从 Agent status 或 terminal text 推断。
...
`runtimeProfile.*` 领域把普通会话选择与管理配置分开。`runtimeProfile.catalog` 只返回 profile id、Provider／模型标签、默认选择、Provider 可用性、快照 schema 兼容性和不含路径的 route identity。`describe`、profile 与 route 写入、默认选择和 `probe` 会暴露或操作可执行文件策略，因此必须经过 loopback 信任栅栏。可信文档包含完整的非秘密有效配置和不含值的凭据状态；它绝不包含凭据值。写入携带 `expectedRevision`，并在发布前校验完整的 profile／route／default 文档。移除用户层条目可能让同 id 的组合基础层条目重新出现。`session.create({ runtimeProfile })` 在 Session 发布前解析并固定该 profile，Session 摘要和 `host/session-added` 则只报告固定的 profile id。外部 profile 会省略默认 Agent Preset 与 Native 模型默认值；显式请求 Agent Preset 会以 `runtime-profile-error` 失败。
