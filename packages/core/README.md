...
`scope` supplies the shared scoping primitive. `agent` owns the public Agent contract; `agent-runtime` defines configurable runtime Providers, `agent-runtime-router` consumes them through the sole factory, and `agent-loop` provides Native execution. `agent-default-model` owns the deployment selection an Agent entry point uses only when a session has no selection of its own.
