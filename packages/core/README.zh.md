...
`scope` 提供共享作用域原语。`agent` 负责公开 Agent 约定；`agent-runtime` 定义可配置运行时 Provider，`agent-runtime-router` 通过唯一 Factory 消费它们，`agent-loop` 提供 Native 执行。`agent-default-model` 负责部署选择，Agent 入口仅在会话自身没有选择时使用它。
