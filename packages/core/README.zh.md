...
| [`agent-runtime-acp/`](agent-runtime-acp/README.md) | ACP 协议 v1 一次性运行时 Provider | 注册到 `ctx.agentRuntimes` |
| [`agent-runtime-codex/`](agent-runtime-codex/README.md) | Codex App Server 运行时 Provider | 注册到 `ctx.agentRuntimes` |
...
`scope` 提供共享作用域原语。`agent` 负责公开 Agent 约定；`agent-runtime` 定义可配置运行时 Provider，`agent-runtime-acp` 和 `agent-runtime-codex` 实现外部协议，`agent-runtime-profile` 解析不可变的非秘密设置快照，`agent-runtime-launcher` 根据这些快照启动外部运行时进程树，`agent-runtime-router` 通过唯一 Factory 消费 Provider，`agent-loop` 提供 Native 执行。`agent-default-model` 负责部署选择，Agent 入口仅在会话自身没有选择时使用它。
