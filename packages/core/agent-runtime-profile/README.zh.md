...
本包负责校验配置并解析策略输入；它不执行命令、不构造子进程环境、不执行沙箱，也不写入 Session Header。[Router](../agent-runtime-router/README.md)负责持久化已解析快照，[安全 Launcher](../agent-runtime-launcher/README.md)与 Provider 负责启动和协议行为。
...
- **不保留 profile 历史** - 即使同名 Settings profile 已修改或消失，resume 仍使用 Session Header 中的快照。
