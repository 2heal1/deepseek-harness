...
The package validates configuration and resolves policy inputs; it does not execute commands, construct a child environment, enforce a sandbox, or persist snapshots into Session headers. Those responsibilities belong to the secure launcher, Provider, and Session integration.
...
- **Launch policy is data only** - F4 owns executable resolution, exact environment construction, reserved arguments and variables, sandbox enforcement, process-tree teardown, and redaction.
