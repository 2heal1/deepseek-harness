...
- `session.header: SessionHeader` — detached, deep-frozen creation metadata (`version`, `id`, `createdAt`, optional `cwd`/`parentSession`/`seedLength`/`delegationDepth`/`runtimeProfile`). Construction validates the durable record and requires its id to match `session.id`. The Router stores the complete non-secret effective Runtime Profile snapshot here; resume never substitutes current Settings.
...
- Replay/fork: `create(id, { seed })` validates and freezes a contiguous current-format log and rebuilds its surface; request headers require provider/model, while assistant messages require model provenance or explicit runtime protocol provenance. Persistence owns read compatibility before constructing this current-format seed. `fork(source, boundary?, childSessionId?)` selects a completed-turn prefix, copies `runtimeProfile` by value, excludes `agent/runtime/facts`, remaps retained event references, and records lineage.
