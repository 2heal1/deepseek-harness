/** Build the ordinary npm/Git/path DSH Bundle artifact. */
...
function declarationPath(dir: string, stem: string): string {
  /* v8 ignore next -- fixedExtension:false emits .d.ts; .d.mts accepts a future tsdown equivalent. */
  const candidate = readdirSync(dir).find(name => name === `${stem}.d.ts` || name === `${stem}.d.mts`)
  /* v8 ignore next -- a successful tsdown declaration build always emits the requested entry stem. */
...
async function buildClientPackage(project: BundleProject, outDir: string): Promise<void> {
  ...
  /* v8 ignore next -- Webpack returns assets whenever the assets field is requested. */
  const javascript = (assets ?? []).filter(asset => asset.name.endsWith('.js'))
  if (javascript.length !== 1) {
    throw new Error('dsh-bundle: package browser build must emit one client.js; dynamic-import chunks require the remote target')
  }
}
...
function packageManifest(project: BundleProject): Record<string, unknown> {
  ...
  source.files = [
    'index.js',
    'index.d.ts',
    ...(project.clientEntry === undefined ? [] : ['client.js', 'client.js.map', 'client.d.ts']),
    'cordis.patch.yml',
  ]
...
function publishPackageRoot(stagingDir: string, outDir: string): void {
  ...
  for (const name of readdirSync(stagingDir)) {
    renameSync(join(stagingDir, name), join(outDir, name))
  }
}

/**
 * Build a ready-to-install package Bundle at `<outDir>` while preserving its remote artifacts.
 * @param project - normalized Bundle project.
 * @returns the package artifact directory.
 */
