...
function declaresBrowserPlugin(require: NodeJS.Require, request: string): boolean {
  ...
  // resolve.paths returns null only for Node builtins, which cannot pass the module-resolution check above.
  /* v8 ignore next */
  ...
  try {
    ...
    /* v8 ignore next -- require.resolve(entry) already read this package manifest; only a concurrent host write can invalidate it here. */
    return false
  }
  /* v8 ignore next -- Node resolves package entries only from object package manifests. */
...
export function lintBundleProject(project: BundleProject): void {
  ...
  for (const [specifier, entry] of project.modules) {
    if (entry.startsWith('/') || /^[A-Za-z]:[/\\]/.test(entry)) {
      if (!existsSync(entry)) throw new Error(`dsh-bundle: module ${JSON.stringify(specifier)} not found: ${entry}`)
      continue
    }
    try {
 ...
 * @param overrides - command-line project and configuration overrides.
 ...
 * @param overrides - command-line project and configuration overrides.
 * @returns emitted artifact paths.
 */
