...
- `envMode` 默认为 `scrubbed-parent`：`scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 会丢弃 ambient 中形似凭据的名称与 `DSH_*` 名称，再合并显式 `env` 条目。普通进程调用方也可以选择 `exact`，此时 `env` 是完整子进程环境，且每个值都必须是字符串。终端 spawn 保持 scrubbed-parent 行为；拥有自身 spawn 的 SDK 管理传输可直接导入共享清理逻辑。
