...
- `envMode` defaults to `scrubbed-parent`: `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` drop ambient credential-shaped and `DSH_*` names, then explicit `env` entries merge after the scrub. Ordinary process callers may instead select `exact`, where `env` is the complete child environment and every value must be a string. Terminal spawns retain scrubbed-parent behavior; SDK-managed transports that own their spawn may import the shared scrub directly.
