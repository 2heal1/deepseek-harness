...
Host 端监听 Web index injection 事件，把 `ctx.remoteBundles.web()` 序列化成 `window.__DSH_REMOTE_BUNDLES__`。没有选中远程组合包时它保持空闲。浏览器端插入经典 remote-entry 脚本，每个入口 URL 只初始化一次容器，从 `ctx.modules` 构建 Module Federation share scope，获取固定的 `./client` 导出，并把它挂载为 Cordis 子插件。因此，Cordis 生命周期会与 Web 树的其他部分一起卸载远程插件。
...
- [DSH 组合包构建器](../../bundle/bundle-builder/README.zh.md) — 生成本包消费的浏览器 remote。
...
- **每个远程组合包只有一个客户端导出** — 容器必须暴露构建器固定的 `./client` 模块。
