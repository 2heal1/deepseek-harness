...
Web subagent feature owner: contributes the runtime Activity target to `conversation.view`, the lazily expandable catalog tree to `conversation.session.header.actions`, reason-specific read-only replacements to the conversation composer chain, and the existing `@` reference source to `ctx.inputTriggers`.
...
The Activity tab reads current process facts from the session summary's `runtimeStatus` projection and independently folds durable `agent/runtime/facts`, `agent/runtime/activity`, error `turn/end`, and rejected not-started `agent/submission/settled` events into an ordered ledger. It labels profile- and protocol-sourced facts, preserves each runtime activity's declared `complete` or `partial` fidelity, and renders only the bounded JSON data carried by the event. Missing product, protocol, external-session, capability, argument, diff, usage, and terminal details remain absent instead of being inferred from prose. The current transient Agent error is displayed separately from durable records. The [runtime Activity presentation Agent Note](../../../.agents/notes/implemented/feature/2026-09-16-runtime-activity-web-presentation.md) owns this distinction.
...
- **The catalog has no durable outcome** — catalog activity and timing do not distinguish completion, failure, or cancellation. The Activity ledger can show persisted structured failures, but it does not derive one outcome for the child or expose an Activation identity; stopping is limited to the composer's current-turn Stop for a running continuable child.
