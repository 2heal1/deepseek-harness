...
Web subagent 功能 owner：向 `conversation.view` 贡献运行时 Activity target，向 `conversation.session.header.actions` 贡献可懒加载展开的目录树，向会话编辑器链贡献按原因区分的只读替代呈现，并保留注册到 `ctx.inputTriggers` 的既有 `@` 引用 source。
...
Activity 标签页从会话摘要的 `runtimeStatus` 投影读取当前进程事实，并独立地把持久化的 `agent/runtime/facts`、`agent/runtime/activity`、错误 `turn/end` 和尚未启动即被拒绝的 `agent/submission/settled` 事件折叠为有序记录。它标注来自 profile 与协议的事实，保留每项运行时活动声明的 `complete` 或 `partial` 完整度，并且只渲染事件携带的有界 JSON 数据。缺失的产品、协议、外部会话、能力、参数、diff、用量和终态细节保持缺失，不会根据文案推断。当前瞬态 Agent 错误与持久化记录分开显示。[运行时 Activity 展示 Agent Note](../../../.agents/notes/implemented/feature/2026-09-16-runtime-activity-web-presentation.md)规定了这一区分。
...
- **目录没有持久化结果**：目录活动状态与计时无法区分完成、失败或取消。Activity 记录可以显示持久化的结构化失败，但不会为 child 推导单一结果，也不公开 Activation 身份；停止能力仅限编辑器上针对运行中可继续 child 的当前轮次 Stop。
