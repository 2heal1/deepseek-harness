...
Package Bundle names resolve from the dsh installation first, then from the profile directory. In-box bundles (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`, `@deepseek-ai/dsh-sdk-app`, `@deepseek-ai/dsh-sdk-minimal`, `@deepseek-ai/dsh-acp-app`) therefore always come from the same installation as the running `dsh`; out-of-tree package Bundles come from the profile's pnpm-managed `node_modules`. A remote source contains `{ type: "remote", name, url }`; startup fetches and validates that stable manifest, loads its immutable Node build with the running installation's shared peers, and applies its patch at the same list position. A bare plugin `name` in any package patch row resolves through the profile directory's Node parent walk, which reaches the maintained installation fallback `$DSH_HOME/profiles/node_modules`. Plain Node installations place one healed symlink there per dependency-closure package. A pkg executable instead places a real ESM proxy that mirrors explicit exports and re-exports the virtual package URL, because operating-system symlinks cannot enter pkg's `/snapshot` filesystem. Every launch also links packages carried only by selected external package Bundles through a dsh-owned directory into the current profile's `node_modules`; existing pnpm entries win, and each profile owns its links independently.
...
`dsh plugin --profile <name> <args...>` initializes the profile when missing (shipped template, or `@deepseek-ai/dsh-base` alone for other names). Package operations pass to `pnpm` with the profile directory as working directory — `add`, `remove`, `why`, `update`, and every other pnpm verb work unchanged; pnpm must be on PATH for those operations. Relative path specs (`.`, `../plugin`, and their `file:`/`link:` forms) are anchored to the invoking directory first, so `add .` from a Bundle checkout installs that checkout, not the profile. After every successful pnpm run, `dsh.profile.bundles` is reconciled against the installed state: each dependency resolving to a package whose manifest declares `"dsh": { "bundle": { "patch": "./cordis.patch.yml" } }` joins the layer stack (so an `update` that gains the declaration activates it), a bundle-less dependency stays plain with a one-time warning, and a removed dependency leaves the stack.

The explicit `name@https://…/dsh-bundle.json` form adds a remote Bundle without invoking pnpm. `name` must be an npm-style unscoped or scoped package name and must equal the fetched manifest's `name`; adding the same remote name replaces its URL, while a package Bundle with that name must be removed before changing delivery forms. `dsh plugin --profile <name> remove <name>` removes a matching remote source without pnpm. Package and remote additions may share one command: pnpm must succeed before the remote sources are committed. A remote add performs one validation fetch; each later Profile start fetches the stable manifest again and selects the build it names. There is no background polling or live replacement.
...
Install external package Bundles through `dsh plugin --profile <name> add <package-or-git-spec>`, or add a Builder-produced remote Bundle through `name@https://…/dsh-bundle.json`. A package owns its installed dependencies; a remote artifact contains its non-peer runtime dependencies and consumes declared peers from the Host. Both contribute their declared `cordis.patch.yml` layer. The CLI also ships `@deepseek-ai/dsh-mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.
