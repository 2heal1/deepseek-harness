...
The [DSH Bundle Builder](../packages/bundle/bundle-builder/README.md) emits package and remote forms from one source project. The runtime protocol has no updater, authentication layer, or TypeScript declaration transport: a restart selects the current remote build, ordinary HTTP infrastructure controls access, and the package form supplies compile-time types.
