...
| S1 | 运行时支持的一次性子 agent | F3、F4、D2 | M | 中等，高级评审 | `in-progress` | `feat/configurable-cli-s1-child-agent` | — | Runtime Provider route、detached child Session、私有 Agent scope、容量、工作区、凭据隔离、取消与完全停稳；ACP 与 route 60 项逐文件 100% coverage，真实 Loader snapshot、`doc-sync` 28 项、build、lint 与 hygiene 通过；等待提交、推送和 Draft PR |
