...
| G2 | Activity 与子 agent 树展示 | M1、S1、F5 | G | 中等；固定渲染 fixture 可交给较低 agent | `in-progress` | `feat/configurable-cli-g2-activity-ui` | — | Activity target 展示当前进程事实、持久化 Provider 活动完整度与结构化失败，并复用递归 subagent 目录展示关系；ui-subagent 27 项逐文件 100% coverage、组装 Web 快照、build、`doc-sync` 28 项、lint 与 hygiene 通过，等待提交、推送和 Draft PR |
