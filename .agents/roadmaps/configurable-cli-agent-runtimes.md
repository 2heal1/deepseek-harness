...
| P0c | Host、SDK、事件与威胁模型审计 | D0 | P0 | 高级 | `in-progress` | `feat/configurable-cli-p0c-runtime-audit` | — | [审计报告](configurable-cli-agent-runtimes-p0c-audit.md)：调用方行为矩阵、事件表、启动威胁模型与 P1 冻结输入；`doc-sync` 28 项、lint、`git diff --check` 通过 |
