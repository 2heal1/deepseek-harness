...
| D2 | ACP 一次性 Provider | P0b、F1、F4 | D | 中等，高级状态机评审 | `in-progress` | `feat/configurable-cli-d2-acp-provider` | — | P0b 官方 SDK fixture、一次性结果、Driver argv 与保留参数拒绝、取消／协议失败／完全停稳、bounded stdout／stderr／assistant output、真实 Loader + Router 组合；ACP 55 项及 Router 相关 1458 项测试覆盖改动源文件 100%，`doc-sync` 28 项、build、lint 与 hygiene 通过 |
