...
当选中构建包含 Web 入口时，Web profile 的 remote 桥接器只把不可变描述序列化到页面。浏览器直接从发布方加载 remote entry 和 chunks，使用运行中客户端模块系统里的 Cordis 与其他声明模块初始化容器，再把固定的 `./client` 导出挂载为普通子插件。系统不提供 Node 代理、认证注入或远程更新器。

`@deepseek-ai/dsh-bundle-builder` 是作者使用的入口。它的约定输入是 `package.json`、`cordis.patch.yml`、`src/index.ts` 和可选的 `src/client/index.ts`。默认 `dual` target 会把普通 npm/Git/path 组合包及其 TypeScript 声明直接写入 `dist/`，并把可执行 Node/浏览器远程产物和不可变构建目录写入 `dist/remote/`；`package` 与 `remote` 可单独选择一种形式。Module Federation 是实现细节，不贡献面向用户的标识或配置。
...
- **内置认证和自动更新**：凭据注入必须定义跨域浏览器行为、secret 归属、日志脱敏和 chunk 请求策略。自动替换必须定义兼容性、回滚和活跃 fiber 迁移。二者都不属于首版协议。发布方提供普通 HTTP(S)，用户或进程管理器选择重启边界。
- **在 DSH 配置中暴露 Module Federation 名称**：容器名和 exposed module key 都由构建器派生并固定。暴露它们只会把内部传输方式变成产品词汇，并不能新增 DSH 用例。
