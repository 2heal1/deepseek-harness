...
One backend, two dialects: `shellDialect: 'bash' | 'pwsh'` (default `'bash'`, existing deployments byte-identical). The effective `shellPath`/`shellArgs` resolve per dialect (bash `/bin/bash --noprofile --norc -i`; pwsh through the shared `dsh-pwsh-local` resolver with `-NoLogo -NoProfile`, keeping the interactive host for child REPLs). The child environment drops the bash-only `PS1`/`PROMPT_COMMAND` markers and adds `NO_COLOR` for pwsh. pwsh cannot install its prompt from the environment, so the backend waits for the native prompt before writing the prompt function through the session, then waits until the controlled prompt is actually visible; either prompt gap can outlast the silence bound, so readiness may loop over follow-up sends, while `session_exit` or `timeout` rejects the spawn. PowerShell's line editor can issue a `CSI 6n` cursor-position query before accepting input; the sanitizer recognizes complete or split queries, removes them from output, and the session responds with the fixed `CSI 1;1R` position expected by this non-emulating terminal. Both dialects emit the same BEL-terminated OSC `133;D;` marker, so the sanitizer, `PROMPT_MARKER_PREFIX`, `CONTROLLED_PROMPT`, and the exact-tail readiness logic are reused untouched — the marker stays a readiness signal with an unconsumed payload, exactly as in the bash path, and no model-notification channel was added (aligned with the current implementation; the deferred BEL event channel stays deferred). For a stable foreground process group, exact stdin-wait readiness requires a non-wait observation after the provider write followed by a wait observation; the pre-write state cannot prove that pwsh consumed the new command.
