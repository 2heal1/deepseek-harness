...
每个拉取请求还会启动一个常规且独立的 `windows-native` 作业，名称为 `windows node 24 / native complete`。规范仓库使用组织自有的 `dsh-windows-2025-16core` 运行器，并为覆盖率与外层门禁各配置两个工作线程；fork 使用标准 `windows-2025`，并为覆盖率、外层门禁与发布检查各配置一个工作线程。该作业为工作区符号链接启用开发人员模式，通过 `pnpm/action-setup` 提供仓库固定版本的 pnpm，在不传输 store 归档的情况下执行不可变安装，并在原生 PowerShell 下运行 `pnpm run check:ci:windows-complete`。门禁卡住时，120 分钟超时会为其设定上限，同时不把实测性能目标当作正确性截止时间。
...
**在规范仓库中使用 GitHub 标准的 `windows-2025` 运行器。** 该可移植双核镜像能可靠完成这份完整清单，但其 32 分钟的串行结果使自动原生信号在该仓库中的实用性远低于所选的 16 核运行器。fork 无法继承组织自有的运行器池，因此接受这项延迟。
...
维护者必须保留两种有意设计的执行拓扑：Wine 快照使用 Linux 安装加 hoisted 布局来触达 win32 二进制文件，而原生作业在规范仓库的组织自有 16 核 Windows 运行器或 fork 的标准串行运行器上使用不可变工作区。任一作业独有的失败都必须依据这项平台拆分分类，不得削弱或静默跳过。
