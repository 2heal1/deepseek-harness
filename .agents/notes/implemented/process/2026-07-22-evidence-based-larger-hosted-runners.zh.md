...
企业保留仅限本仓库使用的 Ubuntu 和 Windows x64 大型运行器池。`deepseek-ai/deepseek-harness` 中的拉取请求在 16 核 Ubuntu 24.04 池上运行 3 个 Linux 主作业，并在 16 核 Windows 2025 池上运行独立的原生 Windows 信号。fork 不会继承组织自有的运行器组，因此会以较低的 worker 预算在 GitHub 标准托管运行器上执行相同命令。必需的 Wine 信号仍位于标准托管 Linux。企业级运行器池已禁用公网 IP；工作流并发仍设有边界，因为自动扩缩容上限既不会分配闲置机器，也不意味着仓库工作可以无限扩展。

规范仓库的必需主路径依赖企业级 Linux 运行器池；fork 拉取请求使用[可移植恢复决策](2026-07-23-portable-required-pull-request-ci.md)规定的标准运行器路径。GitHub 标准托管作业保留 Node 22.19、Node 26 和 Python SDK 兼容性约定，而[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.md)则在 `master` 上通过自托管热备池持续提供完整聚合流程证据。`suite=larger-runner-benchmark` 比较已预配规格上相互独立的关键通道，`suite=consolidated-runner-benchmark` 则比较完整聚合流程。每项基准测试都会先报告实测的处理器和内存容量，再运行仓库工作。
...
[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.md)把必需的构建与生产网站判定保留在标准托管 Linux 上的 Wine 中。独立且不阻断的原生作业通过一次 Windows 设置共同执行工作区构建、生产网站验证、受支持源码覆盖率与完整的可移植性清单；规范仓库使用 16 核大型运行器，fork 使用标准 `windows-2025`。重复的静态检查、文档、包、构建产物、lint 与快照检查由 Linux 提供阻断性判定，原生聚合流程则保留这些观测性检查。
...
自托管的 Linux 与 Windows 串行热备参考，以及被禁用的 `serial-macos` 任务仍然存在。规范仓库的拉取请求使用企业级运行器必需路径和标准托管兼容性作业；fork 使用标准运行器后备路径，其他大型运行器规格仅通过手动触发运行。
...
规范仓库主拓扑中的每个 16 核通道只承担 1 轮设置开销，且不保留工作流级分片 job 或选择器。进程本地 coverage 分区共享这 1 轮设置与同一个工作区。该仓库的拉取请求会消耗付费的企业级 Linux 和 Windows runner 分钟数；fork 使用工作线程预算较低的标准运行器容量，只有在重新测量有价值时，手动基准测试才会加入其他规格。
...
企业级运行器标签缺失或改名时，规范仓库的必需主作业会持续排队，直到启用自托管故障切换；fork 不会引用这些标签。标准托管兼容性作业与 `master` 参考流程仍会报告有用证据，但不能替代缺失的规范仓库必需结果。
