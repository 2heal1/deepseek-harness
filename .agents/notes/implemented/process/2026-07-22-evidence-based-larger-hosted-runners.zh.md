...
企业保留仅限本仓库使用的 Ubuntu 和 Windows x64 大型运行器池。`deepseek-ai/deepseek-harness` 中的拉取请求为三项主 Linux 作业和独立原生 Windows 作业直接指定所选运行器池。fork 不会继承组织自有的运行器组，因此使用串行工作线程预算，在 GitHub 标准托管运行器上执行相同命令。企业级运行器池已禁用公网 IP；工作流并发仍设有边界，因为自动扩缩容上限既不会分配闲置机器，也不意味着仓库工作可以无限扩展。

规范仓库的必需主路径依赖企业级 Linux 运行器池；fork 拉取请求使用[可移植恢复决策](2026-07-23-portable-required-pull-request-ci.md)规定的标准运行器路径。GitHub 标准托管作业保留 Node 22.19、Node 26 和 Python SDK 兼容性约定，而[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.md)保留与运行器无关的完整定义。`suite=larger-runner-benchmark` 比较已预配规格上相互独立的关键通道，`suite=consolidated-runner-benchmark` 则比较完整聚合流程。每项基准测试都会先报告实测的处理器和内存容量，再运行仓库工作。
...
在规范仓库的大型运行器拓扑中，独立原生 Windows 作业通过一次环境设置承载阻塞性构建、生产网站、覆盖率与观测性构建产物检查，重复的 lint 与快照清单则由 Linux 负责。[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.md)让这项原生结果保持在必需聚合流程之外，并为 fork 提供标准 `windows-2025` 路径，且不延长规范仓库的必需路径。
...
只有在 `master` 移动时，才运行完整的 Linux、macOS 和 Windows 串行参考。规范仓库的拉取请求使用企业级主路径和标准托管兼容性作业；fork 使用标准运行器后备路径，其他大型运行器规格仅通过手动触发运行。
...
规范仓库主拓扑中的每条大型运行器通道只承担 1 轮设置开销，且不保留分片选择器。该仓库的拉取请求会消耗付费的企业级 Linux 和 Windows 运行器分钟数；fork 使用带串行预算的标准运行器容量，只有在重新测量有价值时，手动基准测试才会加入其他规格。
...
企业级运行器标签缺失或改名时，规范仓库的必需主作业会持续排队，直到启用其自托管故障切换。fork 不会引用这些标签。标准托管兼容性作业与 `master` 参考流程仍会报告有用证据，但不能替代规范仓库缺失的必需结果。
