...
The portable reference uses GitHub's standard `ubuntu-latest`, `macos-latest`, and `windows-2025` labels. The required pull-request Windows job runs under Wine on `ubuntu-latest`, while the independent pull-request native job uses `dsh-windows-2025-16core` in the canonical repository, `windows-2025` in forks, and the self-hosted `[self-hosted, dsh-win-ci, windows]` pool under explicit failover (see the [failover runbook](2026-07-26-ci-failover-runbook.md)). It remains absent from the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md); when enabled, `serial / windows` is a second complete, unsharded native-kernel oracle. The [required-CI decision](2026-07-23-portable-required-pull-request-ci.md) keeps fork correctness paths runnable without repository-external runner configuration.
