...
The enterprise keeps repo-restricted x64 larger-runner pools for Ubuntu and Windows. Pull requests in `deepseek-ai/deepseek-harness` name the selected pools directly for the three primary Linux jobs and the independent native Windows job. Forks run the same commands on standard GitHub-hosted runners with serial worker budgets because organization-owned runner groups are not inherited. Public IPs are disabled on the enterprise pools, and workflow concurrency remains bounded because an autoscaling ceiling neither allocates idle machines nor makes repository work scale without limit.

The canonical repository's required primary path depends on the enterprise Linux pool; fork pull requests use the portable standard-runner path from the [portable recovery decision](2026-07-23-portable-required-pull-request-ci.md). Standard GitHub-hosted jobs retain the Node 22.19, Node 26, and Python SDK compatibility contracts, while the [serial reference](2026-07-21-serial-cross-platform-ci-reference.md) keeps complete runner-independent definitions. `suite=larger-runner-benchmark` compares isolated critical lanes across provisioned sizes, and `suite=consolidated-runner-benchmark` compares whole aggregates. Each benchmark reports its observed processor and memory capacity before running repository work.
...
Within the canonical repository's larger-runner topology, the independent native Windows job shares one setup across the blocking build and production site plus coverage and observational built-artifact checks, while Linux owns the duplicate lint and snapshot inventories. The [dual Windows pull-request topology](2026-08-08-native-windows-pull-request-ci.md) keeps this native result outside the required aggregate and gives forks a standard `windows-2025` route without extending the canonical repository's required path.
...
Complete serial Linux, macOS, and Windows references run only when `master` moves. Canonical-repository pull requests use the enterprise primary path plus standard-hosted compatibility jobs; forks use the standard-runner fallback, while other larger-runner sizes run only by manual dispatch.
...
The canonical repository's primary topology pays one setup wave per larger-runner lane and retains no shard selectors. Its pull requests consume paid enterprise Linux and Windows minutes; forks consume standard-runner capacity with serial budgets, and manual benchmarks add other sizes only when remeasurement is useful.
...
Missing or renamed enterprise labels leave the canonical repository's required primary jobs queued until its self-hosted failover is activated. Forks do not reference those labels. Standard-hosted compatibility jobs and `master` references still report useful evidence, but they do not substitute for a missing canonical required result.
