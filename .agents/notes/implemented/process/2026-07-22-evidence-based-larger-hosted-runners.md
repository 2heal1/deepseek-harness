...
The self-hosted serial Linux and Windows standby references and the disabled `serial-macos` job exist. Canonical-repository pull requests use the enterprise required path plus standard-hosted compatibility jobs; forks use the standard-runner fallback, while other larger-runner sizes run only by manual dispatch.
...
The canonical repository's primary topology pays one setup wave per 16-core lane and retains no workflow-level shard jobs or selectors. Process-local coverage partitions share that one setup and workspace. Its pull requests consume paid enterprise Linux and Windows minutes; forks consume standard-runner capacity with reduced worker budgets, and manual benchmarks add other sizes only when remeasurement is useful.
