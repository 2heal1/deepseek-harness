...
Product-native commands, tools, file edits, and diffs use runtime-owned activity events. They do not masquerade as Harness `tool/call` or `tool/result` events and do not enter derived model history, because Harness did not execute those tools and may not know their complete model-visible inputs. Tools invoked through the Harness gateway execute through `ToolRuntime` and preserve its approval and policy behavior, while non-surface `agent/runtime/tool-call` and `agent/runtime/tool-result` events record external-runtime provenance without inventing a Native step.
...
The existing three-column Web shell remains the first client. Runtime Profiles and Subagent Routes have dedicated settings forms and probe diagnostics. The new-session selector creates a Session with an explicit profile instead of changing an already published blank Agent, and the conversation header reads the profile id pinned in the Session Header. The Activity tab presents current process facts and capabilities plus durable provider activity and structured failures, while the existing recursive subagent catalog presents child relationships. It preserves declared fidelity and does not infer missing model, argument, diff, usage, or terminal details. Future diff, terminal, image, model, steering, approval, and resume controls appear only for declared capabilities, while Host methods independently enforce the same checks.
